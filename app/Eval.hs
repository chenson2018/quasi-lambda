...
occurs :: Exp -> [Var]
occurs [ex| $v:x |] = [x]
occurs [ex| λ$v:v . $e:e|] = v : occurs e
...
subst e' x' y' = subst' (allFresh \\ occurs e' `union` occurs y') e' x' y'
  where
    ...
    subst' _ e@[ex| $v:v|] x y
      | v == x = y
      | otherwise = e
    subst' fresh e@[ex| λ$v:v . $e:body|] x y
      | v == x = e
      | otherwise =
...
call_by_value [ex|$e:e1 $e:e2|] =
  case call_by_value e1 of
    [ex|λ$v:v . $e:body|] -> call_by_value (subst body v e2)
...
full_beta [ex|$e:e1 $e:e2|] =
  case full_beta e1 of
    [ex|λ$v:v . $e:body|] -> full_beta (subst body v e2)
    e' -> App e' (full_beta e2)
full_beta [ex| λ$v:v . $e:body|] = Lam v (full_beta body)
full_beta e = e
...
pp :: Exp -> String
pp [ex| $v:x |] = vs x
pp [ex| λ$v:x . $e:e |] = printf "λ%s. %s" (vs x) (pp e)
